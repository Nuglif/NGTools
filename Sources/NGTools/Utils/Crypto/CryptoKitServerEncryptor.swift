...
struct CryptoKitServerEncryptor: ServerEncryptor {
    ...
    func encryptForAuthenticationServer(_ text: String, rsaPublicKey: String) throws -> String {
        if #available(iOS 13, *) {
            ...
            let symmetricKey = CryptoAESCK.getAESKey(aesKeyData)

            // 2. Encrypt data
            let encryptedData = try CryptoAESCK.encrypt(text, key: symmetricKey)
            ...
            throw Crypto.CryptoError.cryptoKitNotAvailable
        }
    }
}
